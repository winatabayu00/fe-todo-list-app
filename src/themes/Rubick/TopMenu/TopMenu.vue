...
import fakerData from '@/core/utils/faker'
...
import { watch, reactive, computed, onMounted, ref, provide, onBeforeUnmount } from 'vue'
...
onMounted(async () => {
  try {
    // Cek apakah sudah ada menu atau sedang loading
    if (!menuStore.hasMenus && !menuStore.isLoading) {
...
onBeforeUnmount(() => {
  // Cleanup jika komponen di-unmount sebelum request selesai
  menuStore.cancelRequest()
})
      ...
      <div class="flex items-center h-[70px] z-[51] relative">
        ...
        <Menu>
          ...
          >
            ...
            <Menu.Item class="hover:bg-white/5" @click="$router.push({ path: '/user/profiles' })">
              <Lucide icon="User" class="w-4 h-4 mr-2" />
              Profile
            </Menu.Item>
            <Menu.Item class="hover:bg-white/5" @click="$router.push({ path: '/user/change-password' })">
    ...
    <nav class="relative z-50 hidden top-nav md:block">
      <ul class="pb-3 xl:pb-0 xl:px-[50px] flex flex-wrap">
        ...
        <template v-else-if="menuStore.error">
          <li class="flex items-center px-5 py-2 text-red-400">
